...
import teksol.schemacode.config.*;
import teksol.schemacode.mimex.BlockType;
import teksol.schemacode.schema.Block;
import teksol.schemacode.schema.BlockPositionMap;
...
public class SchematicsIO {
    ...
    public static void write(Schematic build, OutputStream output) throws IOException {
        BlockPositionMap<Block> map = BlockPositionMap.builderToMindustry(m -> {
        }, build.blocks());
    ...
    public static Schematic read(InputStream input) throws IOException {
        ...
        BlockPositionMap<Block> map = BlockPositionMap.mindustryToBuilder(m -> {
        }, msch.blocks());
    ...
    public static Schematic readMsch(InputStream input) throws IOException {
        ...
        int index = 0;
        String nameFortype = "";
        BlockType blockType = BlockType.forName("@" + FALLBACK.getOrDefault(nameFortype, nameFortype));
        Position position = new Position(0, 0);

        Direction direction = Direction.convert((byte) 0);

        Configuration config = convert(blockType, position, null);
    ...
    public static Configuration convert(BlockType blockType, Position position, Configuration raw) {
        ...
        return switch (blockType.configurationType()) {
            ...
            default ->
                    throw new SchematicsInternalError("Unhandled configuration type %s.", blockType.configurationType());
        };
    }
